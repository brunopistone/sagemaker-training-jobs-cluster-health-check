...
if is_sagemaker_studio; then
    ...
    docker build --network sagemaker --platform linux/amd64 -f ${DOCKERFILE} -t ${REGISTRY}${REPO_NAME}:${TAG} .
else
    echo "Detected local/standard environment - using default network"
    docker build --platform linux/amd64 -f ${DOCKERFILE} -t ${REGISTRY}${REPO_NAME}:${TAG} .
fi
